...
# Align sentiment trend with spend trend loosely
...
# Churn target
# Logit score from weighted risk factors
logit = (
    -1.6                                          # intercept → ~13.5% base rate
...
numeric_cols = [
    'sentiment_score', 'spend_trend', 'num_products', 'tenure_months',
    'escalated_support', 'complaint_count_6m', 'days_since_last_login',
    'feature_adoption_rate', 'auto_renewal_enabled'
]
...
print(f"File size: {pd.io.common.get_handle(output_path, 'rb').handle.seek(0, 2) / 1e6:.1f} MB")
